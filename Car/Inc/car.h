 ...
 */
typedef enum {
  MOTOR_ENCODER_DIRECTION_NONE = 0,
  FORWARD = 1,
  BACKWARD = 2,
...
typedef PWM_SIGNAL SERVO_SIGNAL;
 ...
 * @param  angle: 旋转到的角度，设0为中间，负数为左传角度，反之右转角度为1或-1即为打满舵机
 */
void ServoRotate(SERVO *servo, double angle);
 ...
 */
void DriverPower(DRIVER *driver, double angle, float power_a, float power_b);

/**
 * @brief  提供整个小车的转向 前进后退的操作函数
 * @param  driver: 两个电机以及一个舵机
 * @param  angle: 旋转角度
 * @param  power: 电机输出的功率比
 */
void DriverCar(DRIVER *driver, double angle, float power);
